...
　　　　0<=NCN<=3の時、Add-Forwarding zoneの追加はなし。元々のForwarding zoneのみ
    　　NCN>=4の時、グリッド番号: sg-id + (Nx - 1)　のグリッドをAdd-Forwarding zoneに追加
      　NCN>=5の時、グリッド番号: sg-id - (Nx ＋ 1)　のグリッドをAdd-Forwarding zoneに追加
       NCN>=6の時、グリッド番号: sg-id - 1　のグリッドをAdd-Forwarding zoneに追加
       NCN>=7の時、グリッド番号: sg-id - Nx　のグリッドをAdd-Forwarding zoneに追加
       NCN>=8の時、グリッド番号: sg-id - (Nx - 1)　のグリッドをAdd-Forwarding zoneに追加
